...
func (t *MeowTUI) initUI() {
	...
	titleBar := tview.NewTextView().
		...
		SetText("/ᐠ｡ꞈ｡ᐟ\\ MeowTUI - Purr-fect Kubernetes Dashboard /ᐠ｡ꞈ｡ᐟ\\").
...
func (t *MeowTUI) populateResourceList() {
	...
	for i, resource := range t.resources {
		// Use a closure to capture the correct resource value
		resource := resource // Important: create a local copy
		t.resourceList.AddItem(fmt.Sprintf("📋 %s", resource), "", rune('a'+i), func() {
			t.currentView = resource
			t.loadResourceDetails(resource)
			t.updateStatusBar()
		})
	}
	
	// Force selection of first item if list has items
	if t.resourceList.GetItemCount() > 0 {
		t.resourceList.SetCurrentItem(0)
		// Load details for the first resource immediately
		t.loadResourceDetails(t.resources[0])
	}
}
...
func (t *MeowTUI) loadResourceDetails(resource string) {
	...
	// Execute command with timeout to prevent blocking
	...
	// Wait for command output or timeout
	select {
	case result := <-outputChan:
		if result.err != nil {
			t.detailView.SetText("[" + colorToHex(colorError) + "]😿 Error fetching " + resource + ": " + result.err.Error() + "[" + colorToHex(colorText) + "]\n\n" + string(result.output))
			return
		}
		
		// Format output with colors
		formattedOutput := formatKubectlOutput(string(result.output))
		t.detailView.SetText(formattedOutput)
		
	case <-time.After(5 * time.Second):
		// Timeout occurred
		t.detailView.SetText("[" + colorToHex(colorWarning) + "]⏱️ Taking longer than expected to fetch " + resource + "...\n" +
			"Check if kubectl is configured correctly and your cluster is running.[" + colorToHex(colorText) + "]\n\n" +
